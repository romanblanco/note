...
<?php
  if (mysqli_connect_errno($connection)) {
    echo "<span>Connection to MySQL database failed: ".mysqli_connect_error()."</span>";
  }
  if (isset($_REQUEST['note']) && (isset($_REQUEST['topic']))) {
    if ($_REQUEST['note'] !== '' && $_REQUEST['topic'] !== '') {
      $sqlTask = "INSERT INTO `notes` (`topic`, `note`, `tag`, `date`) VALUES ('$_POST[topic]', '$_POST[note]', '$_POST[tag]', now())";
      if (!mysqli_query($connection, $sqlTask)) {
      die('Error during saving note: ' . mysqli_error($con));
      }
